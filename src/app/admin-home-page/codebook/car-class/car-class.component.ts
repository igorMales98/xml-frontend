  ...
  allCarClasses: CarClass[] = [];
    ...
    ngOnInit(): void {
      ...
      this.editCarClassForm = this.formBuilder.group({
        id: [''],
        carClassNameEdit: ['', [Validators.required, Validators.pattern(/^[A-Za-z.-]*$/), Validators.maxLength(15), Validators.minLength(1)]]
      });
    }
    ...
    addNameToCarClass() {
      const name = new CarClass('', this.carClassForm.value.name);
