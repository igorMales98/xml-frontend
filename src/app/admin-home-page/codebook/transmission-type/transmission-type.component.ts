  ...
  allTransmissionTypes: TransmissionType[] = [];
    ...
    ngOnInit(): void {
      this.transmissionTypeForm = this.formBuilder.group({
        transmissionTypeName: ['', [Validators.required, Validators.pattern(/^[A-Za-z.-]*$/), Validators.maxLength(15), Validators.minLength(1)]]
      });
      ...
      this.editTransmissionTypeForm = this.formBuilder.group({
        id: [''],
        transmissionTypeNameEdit: ['', [Validators.required, Validators.pattern(/^[A-Za-z.-]*$/), Validators.maxLength(15), Validators.minLength(1)]]
      });
    }
    ...
    addNameToTransmissionType() {
      const name = new TransmissionType('', this.transmissionTypeForm.value.name);
