  ...
  allFuelTypes: FuelType[] = [];
    ...
    ngOnInit(): void {
      ...
      this.editFuelTypeForm = this.formBuilder.group({
        id: [''],
        fuelTypeNameEdit: ['', [Validators.required, Validators.pattern(/^[A-Za-z.-]*$/), Validators.maxLength(15), Validators.minLength(1)]]
      });
    }
    ...
    addNameToFuelType() {
      const name = new FuelType('', this.fuelTypeForm.value.name);
